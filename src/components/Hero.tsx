      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-2 lg:items-center">
        <div>
          ...
          <h1 className="mt-4 font-display text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-[3.4rem]">
            ...
            <br />
            This is{" "}
            <span className="text-gradient-pink">{SITE.fullName}</span>,
            <br />
            I'm a{" "}
            <span className="text-gradient-teal">{SITE.role}</span>{" "}
            <span className="text-gradient-teal">{SITE.specialization}</span>.
          </h1>

          <div className="mt-6 flex items-center gap-4 text-neon-pink">
            <a href={SITE.github} target="_blank" rel="noreferrer" aria-label="GitHub" className="transition-colors hover:text-neon-violet"><Github size={20} /></a>
            <a href="#contact" aria-label="LinkedIn" className="transition-colors hover:text-neon-violet"><Linkedin size={20} /></a>
            <a href="#contact" aria-label="Facebook" className="transition-colors hover:text-neon-violet"><Facebook size={20} /></a>
            <a href="#contact" aria-label="Twitter" className="transition-colors hover:text-neon-violet"><Twitter size={20} /></a>
          </div>
...
/* Inline render swap: replace tags above with components via dangerouslySet not allowed. Instead render manually below. */
